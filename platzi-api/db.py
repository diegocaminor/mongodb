...
def crear_carrera(json):
    return str(db.carreras.insert_one(json))


def consultar_carrera_por_id(carrera_id):
    # dumps convierte bson a json
...
def crear_curso(json):
    return dumps(db.cursos.insert_one(json).inserted_id)
...
def actualizar_curso(curso):
    ...
    return str(db.cursos.update_one({'_id': ObjectId(curso['_id'])}, {'$set': {'nombre': curso['nombre'], 'description': curso['description'], 'clases': curso['clases']}}).modified_count)
